...
func (app *IntfApp) translateUpdateLagIntf(d *db.DB, lagName *string, inpOp reqType) ([]db.WatchKeys, error) {
	...
	entryVal.Field["lagid"], err = getVlanIdFromVlanName(lagName)
	if err != nil {
		return keys, err
	}
...
func (app *IntfApp) processUpdateLagIntfConfig(d *db.DB) error {
	...
	for lagId, lagEntry := range app.ifTableMap {
		switch lagEntry.op {
		case opCreate:
			err = d.CreateEntry(app.lagD.lagTs, db.Key{Comp: []string{lagId}}, lagEntry.entry)
			if err != nil {
				errStr := "Creating LAG entry for LAG : " + lagId + " failed"
				return errors.New(errStr)
			}
		case opUpdate:
			err = d.SetEntry(app.lagD.lagTs, db.Key{Comp: []string{lagId}}, lagEntry.entry)
			if err != nil {
				errStr := "Updating LAG entry for LAG : " + lagId + " failed"
				return errors.New(errStr)
			}
		}
	}
